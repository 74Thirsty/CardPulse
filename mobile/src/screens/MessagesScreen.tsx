          ...
          renderItem={({ item }) => (
            ...
            >
              <Text
                ...
                numberOfLines={2}
              >
                {item.listing?.card?.name ?? `Listing ${item.listingId}`}
              </Text>
              <Text style={styles.threadSubtext} numberOfLines={1}>
                {item.listing?.price ? `$${item.listing.price.toFixed(2)} · ` : ''}
            ...
            {selectedThread.listing ? (
              <View style={styles.listingSummary}>
                <Text style={styles.listingTitle}>
                  {selectedThread.listing.card?.name ?? selectedThread.listingId}
                </Text>
                <Text style={styles.listingMeta}>
                  {selectedThread.listing.card?.game ?? 'Unknown'} · {selectedThread.listing.condition}
                </Text>
                {selectedThread.listing.valuation?.average ? (
                  <Text style={styles.listingValuation}>
                    Market avg ${selectedThread.listing.valuation.average.toFixed(2)}
                  </Text>
                ) : null}
              </View>
            ) : null}
