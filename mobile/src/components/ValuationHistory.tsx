    ...
    <View style={styles.container}>
      ...
      <View style={styles.statsRow}>
        ...
        <Stat
          label="7d Change"
          value={
            valuation.change7d !== null
              ? `${valuation.change7d > 0 ? '+' : ''}${valuation.change7d}%`
              : 'N/A'
          }
        />
        <Stat
          label="7d Avg"
          value={valuation.average7d ? `$${valuation.average7d.toFixed(2)}` : 'N/A'}
        />
        <Stat
          label="30d Volume"
          value={valuation.volume30d !== null ? `${valuation.volume30d}` : 'N/A'}
        />
      </View>
      <FlatList
      ...
      {valuation.sources && valuation.sources.length > 0 ? (
        <View style={styles.sources}>
          ...
          {valuation.sources.map((source) => (
            <Text key={source.provider} style={styles.sourceItem}>
              {source.provider}
